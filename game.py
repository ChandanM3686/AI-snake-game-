...
class SnakeGameAI:
    ...
    def _move(self, action):
        ...
        current_dir_index = clockwise.index(self.direction)

        if np.array_equal(action, [1, 0, 0]):  # Move straight
            new_dir = self.direction
        elif np.array_equal(action, [0, 1, 0]):  # Turn right
            new_dir = clockwise[(current_dir_index + 1) % 4]
        else:  # Turn left
            new_dir = counterclockwise[(current_dir_index + 1) % 4]
...
if __name__ == '__main__':
    ...
    game_over = False
    while not game_over:
        action = [1, 0, 0]  # Move straight initially
